...
def boardLoad(board):
    for row in board:
        for cell in row:
            ...
            label.grid(row = cell.row, column = 3)
            label = Label(root, text = "|")
            label.grid(row = cell.row, column = 7)
            # This prints out each row divider
            label = Label(root, text = "=")
            label.grid(row = 3, column = cell.column)
            label = Label(root, text = "=")
            label.grid(row = 7, column = cell.column)
    # The column and row dividers fall 2 cells short, this for loop fixes this issue
    for i in range (1,3):
        label = Label(root, text = "|")
        label.grid(row = 8 + i, column = 3)
        label = Label(root, text = "|")
        label.grid(row = 8 + i, column = 7)
        label = Label(root, text = "=")
        label.grid(row = 3, column = 8 + i)
        label = Label(root, text = "=")
        label.grid(row = 7, column = 8 + i)
